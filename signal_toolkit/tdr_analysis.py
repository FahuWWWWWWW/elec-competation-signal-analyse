...
class TDR:
    ...
    def __init__(self, fs, vf=0.67):
        self.fs = fs
        self.dt = 1.0 / fs
        self.vf = vf
        self.v = c * vf

    def equivalent_sampling(self, real_samples, m, trigger_period):
        dt_real = 1.0 / self.fs
        dt_eff = dt_real / m
    ...
    def detect_reflection(self, signal, threshold=0.05, min_distance=5):
    ...
    def compute_distance(self, delay_samples):
...
def equivalent_sampling(real_waveform, m, trigger_period):
    n = len(real_waveform)
...
def compute_distance(time_delay, vf=0.67):
    return c * vf * time_delay / 2.0


def cable_model(length, fault_type='open', vf=0.67, fs=100e6):
    tdr = TDR(fs=fs, vf=vf)
